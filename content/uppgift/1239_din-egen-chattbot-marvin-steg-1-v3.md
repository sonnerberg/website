...
-----------------------
[INFO]
Taggarna för varje uppgift motsvarar själva menyvalet. För att testa Menyval **2** kan man då använda `--tags=2`, för Menyval **a1** `--tags=a1` osv.
[/INFO]
...
1. Organisera din kod i `if`-satsen inuti `while`-loopen. Samla all kod i filen `marvin.py`. Se till att dina menyval har samma som den **markerade** texten.
